<template>
	<div v-if="productDetails.id">
		<div class="row">
			<div :class="{ 'col-lg-4 col-md-12': $route.name == 'product.details', 'col-lg-5': $route.name != 'product.details' }"
				v-if="lengthCounter(productDetails.media) > 0">
				<div class="product-slider-section slider-arrows">
					<div>
						<!-- <CoolLightBox style="direction: ltr;" :items="productDetails.media.large" :index="index"
							:useZoomBar="true" @close="index = null">
						</CoolLightBox> -->

						<div class="images-wrapper">
							<div class="image" v-for="(image, imageIndex) in productDetails.media" :key="imageIndex"
					...
					<div class="product-image">
						<img @click="index = current_index" :src="productDetails.media[0].url" :alt="productDetails.name">
					</div>

					<VueSlickCarousel v-bind="slick_settings" :rtl="settings.text_direction == 'rtl'">
						<div v-for="(image, small_image_index) in productDetails.media" :key="small_image_index"
					...
					<span class="base" v-if="productDetails.special_discount_check > 0">
						{{
							productDetails.special_discount_type == "flat" ? priceFormat(productDetails.special_discount_check)
				...
				<div class="row justify-content-md-center">
						...
						<div class="product-details-2">
							<div class="product-details-header">
								...
								<div class="sg-rating" v-if="productDetails.rating > 0">
									...
									<span class="rating"> ({{ productDetails.reviews_count }} Review)</span>
								</div>
							</div>
							...
							<!-- <div class="sg-product-price" v-if="!productDetails.is_wholesale">
								<span v-if="productDetails.special_discount_check > 0">{{
									priceFormat(productDetails.discount_percentage) }}</span>
								<span v-else>{{ priceFormat(productDetails.price) }}</span>
							...
							<div class="sg-product-price">
								<span>{{ priceFormat(productDetails.price) }} </span>
							</div>
							<div class="sg-product-color" v-if="productDetails.quantities && productDetails.quantities.length > 0">
								<div class="sg-color">
									<h5>Colors:</h5>
									<div v-for="(item, index) in productDetails.quantities" :key="'color' + index">
										<input type="radio" value="color1" :id="'color' + item.colour.id" v-model="product_form.color_id" :value="item.colour.id" @change="attributeSelect($event.target)"
                           :disabled="attributes.length == 0 && checkDisable(0,color)"/>
										<label :for="'color' + item.colour.id" :class="{ 'disabled_radio' : attributes.length == 0 && checkDisable(0,item.colour) }">
											<span :style="'background:' + item.colour.name"></span>
										</label>
									</div>
								</div>
							</div>
							...
							<span>Taille:</span>
							<div class="sg-product-size" v-for="(data, attribute_index) in productDetails.quantities" :key="'attribute' + attribute_index"  v-if="productDetails.quantities.length > 0">
								<div class="sg-size">
									<form action="#">
										<div v-for="(value, value_index) in productDetails.quantities" :key="'value' + value_index" v-if="value.size.id == data.size.id">
											<input type="radio" :id="data.size.id + '_attribute_' + data.size.id" :value="value.size.id" v-model="product_form.attribute_values[attribute_index]" @change="attributeSelect($event.target,data.size.id,value.size.id)" :disabled="checkDisable(attribute_index,value.size)"/>
											<label :for="data.size.id + '_attribute_' + value.size.id" :class="{ 'disabled_radio' : checkDisable(attribute_index,value.size) }">{{ value.size.name }}</label>
										</div>
									</form>
								</div>
							</div>
							...
							<div class="product-offer product-border" v-if="productDetails.description"
							...
							<div class="product-details-totalPrice product-border"
								...
								<h3>Total Price:
									...
									<span v-else-if="productDetails.is_wholesale != 1">{{
										priceFormat(productDetails.price * product_form.quantity) }}</span>
								...
								<div class="product-cart sg-quantity" v-if="productType()">
									<div class="buttons d-flex align-items-center">
										<a href="javascript:void(0);" class="btn btn-primary"
											@click="addToCart(productDetails,product_form)">
									...
									<div class="buyNowBTN d-flex align-items-center">
										<a href="javascript:void(0)"
											@click="addToCart(productDetails, 1, 1)">
							...
							<div class="buttons" v-else-if="productDetails.is_catalog == 1 && productDetails.external_link">
								<a target="_blank" :href="productDetails.external_link" class="btn btn-primary btn-block">{{
									lang.see_details }}</a>
							</div>
							...
							<div class="product-details-policy product-border mt-4">
								<div class="related-product-shop">
									...
									<div class="related-product-content">
										...
										<h4 v-if="settings.refund_protection_title">{{ settings.refund_protection_title }}
										</h4>
									</div>
							...
							<div class="social-media-icon">
								<ul class="global-list d-flex">
									<li class="fb-icon"><a target="_blank"
											:href="'https://www.facebook.com/sharer/sharer.php?u=' + getUrl('product/' + productDetails.id)"><span
												class="mdi mdi-name mdi-facebook"></span></a></li>
									<li class="tw-icon"><a target="_blank"
											:href="'https://twitter.com/intent/tweet?text=' + productDetails.name + '&url=' + getUrl('product/' + productDetails.id)"><span
												class="mdi mdi-name mdi-twitter"></span></a></li>
									<li class="ld-icon"><a target="_blank"
											:href="'https://www.linkedin.com/sharing/share-offsite?mini=true&url=' + getUrl('product/' + productDetails.id) + '&title=' + productDetails.name + '&summary=Extra+linkedin+summary+can+be+passed+here'"><span
												class="mdi mdi-linkedin"></span></a></li>
									<li class="wh-icon"><a target="_blank"
											:href="'https://wa.me/?text=' + getUrl('product/' + productDetails.id)"><span
					...
					<div v-if="$route.name == 'product.details'" class="col-lg-4 col-md-4">
						<div class="sg-seller-product pb-sm-3">
							...
							<div class="product-widget-recent-entries" v-if="productDetails.length > 0">
				...
				<div class="products-description product-details-description">
					<ul class="nav nav-tabs description-tabs" role="tablist">
						...
						<li class="nav-item" role="presentation"
							...
							<a class="nav-link" :class="{ active: activeNav == 'specifications' }" href="javascript:void(0)"
								@click="activeNav = 'specifications'" aria-controls="specifications" role="tab"
					...
					<div class="tab-content">
						<div role="tabpanel" class="tab-pane fade" :class="{ 'show active': activeNav == 'details' }"
							id="details">
							<div
								v-html="productDetails.description"></div>
							<div class="text-center" v-if="productDetails.media.length > 0">
								<div class="mt-3" v-for="(image, index) in productDetails.media" :key="index">
									<img loading="lazy" :src="image.url" :alt="productDetails.name" />
								</div>
							</div>
						</div>
						<!-- <div role="tabpanel" class="tab-pane fade"
							...
							<div class="specifications"> <iframe :src="productDetails.language_product.specification"
						...
						<div role="tabpanel" class="tab-pane fade" :class="{ 'show active': activeNav == 'delivery' }"
							...
							<div class="sg-reviews">
										...
										<div class="comment_info">
												...
												<div class="comment-title" v-if="review.user">
													...
													<div class="sg-rating">
														<star-rating :rating="review.rating" :read-only="true"
															:star-size="10" active-color="#C9151B"></star-rating>
													</div>
													<div class="comment-meta">
														<ul class="global-list">
															<li><a href="javascript:void(0)">{{ review.review_date }}</a>
															</li>
														</ul>
													</div>
													<a class="float-end" v-if="authUser && review.user_id == authUser.id"
														@click="editReview(review)" href="javascript:void(0)">{{ lang.edit
														}}</a>
												</div>
											</div><!-- /.comment-box -->
											<p>{{ review.comment }}</p>
											<div class="selected-media mt-0 m-2" v-if="review.images">
												<img loading="lazy" :src="review.image_link"
													:alt="productDetails.product_name" class="img-thumbnail" width="100" />
											</div>
												...
												<ul class="global-list" v-if="authUser">
													<li v-if="alreadyLiked(review.review_likes)"><a
															...
															<span class="replies_text">({{ review.review_likes ?
																review.review_likes.length : 0 }})</span></a></li>

													<li v-else><a href="javascript:void(0)" @click="reviewLike(review.id)"
															:class="{ disable_btn: like_loading }">
													...
													<li v-if="review.replies.length > 0"><a href="javascript:void(0)"
											...
											<form @submit.prevent="reviewReply(review.id)" v-if="reply_form == review.id">
												<div class="row">
													...
													<div class="col-lg-2">
														<input v-if="!reply_loading" type="submit" class="btn btn-primary"
															value="Send" />
													...
													<div class="comment_info">
														<div class="commenter-avatar" v-if="reply.user">
															<router-link :to="{ name: 'dashboard' }"><img class="img-fluid"
																	loading="lazy" :src="reply.user.profile_image"
																	:alt="reply.user.full_name" /></router-link>
														</div>
														<div class="comment-box">
															<div class="comment-title">
																<span class="title-1" v-if="reply.user"><router-link
																		:to="{ name: 'dashboard' }" class="url">{{
																			reply.user.full_name }}</router-link></span>
																<div class="comment-meta">
																	<ul class="global-list">
																		<li><a href="javascript:void(0)">{{ reply.reply_date
																		}}</a></li>
																	</ul>
																</div>
															</div>
								...
								<div class="text-center show-more" v-if="reviews.next_page_url">
									<a href="javascript:void(0)" @click="loadReviews()" class="btn btn-primary">{{
										lang.show_more }}</a>
								</div>
							</div>
							...
							<div v-if="(authUser && !productDetails.user_review) || edit" class="review-form">
								...
								<form @submit.prevent="submitReview">
									<div class="form-group">
										...
										<input class="form-control" v-model="product_form.title" name="name" type="text"
...
export default {
	...
	mounted() {
		if (this.productDetails && this.productDetails.form) {
			...
			this.large_image = this.productDetails.media[0].url;
	...
	methods: {
		...
		addToCart(product, buy, is_buy_now) {
