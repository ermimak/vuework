<template>
  <header class="sg-header home-4" :class="{ 'sidebar-active': smCategory, 'mobile-menu-active': show_sm_home }" v-if="siteSettings">
        ...
        <div class="topbar-content">
          <div class="left-content">
            <ul class="global-list d-flex align-items-center">
              <li v-if="siteSettings.settings.language_switcher != 0">
                <div class="dropdown">
              ...
              <li v-if="siteSettings.settings.currency_switcher != 0">
                <div class="dropdown">
              ...
              <li v-if="siteSettings.settings.topbar_play_store_link == 1">
                <a :href="settings.play_store_link" target="_blank"
                ><span class="mdi mdi-name mdi-google-play"></span> <small>App Store</small></a
                >
              </li>
              <li v-if="siteSettings.settings.topbar_app_store_link == 1">
                <a :href="siteSettings.settings.apple_store_link" target="_blank">
                  <span class="mdi mdi-name mdi-apple"></span> <small>Be a Seller</small>
                </a>
              </li>
              <li v-if="siteSettings.settings.seller_system == 1 && !authUser">
                <router-link
                  :to="{
              ...
              <li v-if="addons__.includes('affiliate')">
                <router-link
                  :to="{
          ...
          <div class="right-content new-list">
            <ul class="global-list">
              <li v-if="siteSettings">
                <a :href="'tel:' + siteSettings.settings.header_contact_phone" class="live-chat">
                  <strong>Call : {{ siteSettings.settings.header_contact_phone }}</strong>
                </a>
              </li>
              <li>
              ...
              <li>
                ...
                >Compare <span v-if="compareList > 0">({{ compareList }})</span></router-link
                >
              </li>
              <li>
    ...
    <div :class="addons__.includes('ishopet') ? toggleNavClass() : ''" id="middle_nav" class="header-middle">
      <div class="container">
        <div class="botom-content">
          <div class="sg-logo">
            <div class="sg-categorie-menu categorie-lg align-self-lg-center">
              <div class="top-content" v-if="addons__.includes('ishopet')">
              ...
              <sidebar_categories v-if="addons__ && addons__.includes('ishopet')" ref="sidebar_category"
                                  :home="true"></sidebar_categories>
            </div>
            <router-link :to="{ name: 'home' }">
              <svg v-if="siteSettings.settings.demo_mode" id="Layer_1" xmlns="http://www.w3.org/2000/svg"
              ...
              <img v-else :src="siteSettings.settings.dark_logo" alt="Logo" class="img-fluid"/>
            </router-link>
          </div>
          <div class="sg-search">
              ...
              <li class="d-flex user-log-info">
                ...
                <div class="text-left text" v-if="authUser">
                  ...
                  <a class="d-block" :href="getUrl('seller/dashboard')"
                     v-if="authUser.user_type == 'seller'">Dashboard</a>
                  <a class="d-block" :href="getUrl('admin/dashboard')"
                     v-if="authUser.user_type == 'admin' || authUser.user_type == 'staff'">Dashboard</a>
                  <a href="javascript:void(0)" @click="logout"> {{ lang.logout }}</a>
                </div>
                <div v-else>
                  <router-link class="d-block" :to="{ name: 'login' }">Sign in</router-link>
    ...
    <div class="header-menu" :class="[toggleNavClass(), siteSettings.settings.header_theme]" id="nav"
         v-if="!addons__.includes('ishopet')">
      <div class="container">
          ...
          <div class="right-content">
            <div class="sg-menu d-flex justify-content-between">
              <nav class="navbar navbar-expand-lg">
                <div class="sg-logo">
                  <router-link :to="{ name: 'home' }">
                    <img loading="lazy" :src="siteSettings.settings.light_logo" alt="Logo" class="img-fluid"/>
                  </router-link>
                </div>
    ...
    <div class="sg-categorie-menu home-menu-sm">
      <div class="sg-toggle" @click="$store.commit('setSmCategory', false)"><span class="mdi mdi-name mdi-close"></span>
      </div>
...
export default {
  ...
  computed: {
    languages() {
      let languages = this.siteSettings.languages;
      return languages.length > 0 ? languages : [];
    },
    currencies() {
      if (this.siteSettings) {
        return this.siteSettings.currencies;
      }
    },
    activeLanguage() {
      return this.siteSettings ? this.siteSettings.active_language : null;
    },
    activeCurrency() {
      return this.siteSettings ? this.siteSettings.active_currency : null;
    },
    carts() {
      let carts = this.siteSettings.carts;
      return carts.length > 0 ? carts : [];
    },
    visibleCategory() {
      let categories = this.siteSettings.categories;
    ...
    headerMenu() {
      return this.siteSettings ? this.siteSettings.settings.header_menu : null;
    },
    wishlists() {
      return this.siteSettings ? this.siteSettings.wishlists : null;
    },
    active_modal() {
    ...
    compareList() {
      return this.siteSettings ? this.siteSettings.compare_list : null;
    }
  },
  methods: {
    subMenuToggle(event) {
    ...
    toggleNavClass() {
      return {
        ...
        "sticky-bg": this.addons__.includes('ishopet'),
        "ishopet-header": this.addons__.includes('ishopet'),
      };
    },
