		...
		<section class="grid-view-tab">
			<div class="container">
				<div class="row sg-category-content sg-filter grid-view-tab" :class="activeClass">
					<div class="col-md-4 col-lg-3 filter_area" v-if="lengthCounter(products__) > 0">
									...
									<div id="category" class="accordion-collapse collapse" :class="{ show: category }"
										...
										<div class="accordion-body">
											<ul class="global-list">
												<li v-for="(category, index) in categories__" :key="'category' + index"
											...
											<div v-if="category_load_more || categories__.next_page_url"><a
													@click="loadCategories" href="javaScript:void(0)">Show More</a></div>
										</div>
									</div>
								</div>
												...
												<ul class="global-list">
													<li>
														<a href="javascript:void(0)" @click="$router.go(-1)"><i
																class="mdi mdi-arrow-left"></i>All Categories</a>
													</li>
												</ul>
								...
								<div class="accordion-item" v-if="form.route != 'product.by.brand'">
									<div class="accordion-header" id="ac4">
										<button class="accordion-button" @click="brand = !brand"
											...
											aria-controls="brand_collapse">Brand</button>
									</div>
								...
								<div class="accordion-item">
									<div class="accordion-header" id="ac3">
										<button class="accordion-button" @click="price = !price"
											...
											aria-controls="collapse2">Price </button>
									</div>
								...
								<div class="accordion-item" v-if="settings.color == 1">
									<div class="accordion-header" id="ac5">
										<button class="accordion-button" @click="color = !color"
											...
											aria-controls="collapse5">Color </button>
									</div>
								...
								<div class="accordion-item">
									<div class="accordion-header" id="ac6">
										<button class="accordion-button" @click="ratting = !ratting"
											...
											aria-controls="collapse6">Rating </button>
									</div>
					...
					<div class="col-md-8 col-lg-9">
						...
						<div>
							<div class="mt-2" v-if="products__ && products__.total == 0"></div>
							<product :products="products__" :number="8" :grid_class="'grid-4'"></product>
							<div class="show-more" v-if="products__ && products__.next_page_url && !loading">
								<a href="javaScript:void(0)" @click="loadProduct" class="btn btn-primary">Show More</a>
							</div>
...
export default {
	...
	created() {
		let paramsData = { pagination: true }
		let tag = document.querySelector('meta[name="author"]');
		if (tag) {
			tag.setAttribute("content", this.form.route);
		}
		axios.get(`${this.url2}clothes`, { params: paramsData }).then((response) => {
			this.is_shimmer = true
			if (response.data.error) {
				this.$Progress.fail();
				toastr.error(response.data.error, this.lang.Error + ' !!');
			} else {
				this.products__ = response.data.data.data
			}
		}).catch((error) => {
			this.loading = false;
			this.is_shimmer = true
	...
	methods: {
		...
		productList() {
			axios.get(`${this.url2}products`).then((response) => {
				this.is_shimmer = true
				if (response.data.error) {
					this.$Progress.fail();
					toastr.error(response.data.error, this.lang.Error + ' !!');
				} else {
					this.products__ = response.data.data
				}
			}).catch((error) => {
				this.loading = false;
				this.is_shimmer = true
		...
		categoriesList(){
			axios.get(`${this.url2}categories`).then((response) => {
				this.is_shimmer = true
				if (response.data.error) {
					this.$Progress.fail();
					toastr.error(response.data.error, this.lang.Error + ' !!');
				} else {
					this.categories__ = response.data.data
				}
			}).catch((error) => {
				this.loading = false;
				this.is_shimmer = true
