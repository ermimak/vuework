		...
		<ul class="products" :class="grid_class" v-if="products && products.length > 0">
			<li v-if="products[0] != 'id'" v-for="(product, index) in products" :key="index">
				<div class="sg-product" :class="{ 'style-1': type == 'flash' }">
					<a @click.prevent="routerNavigator('product.details', product.id)">
						<div class="product-thumb">
							<span class="base" v-if="product.special_discount > 0">{{ product.special_discount_type == "flat" ? priceFormat(product.special_discount) + " " + lang.off : product.special_discount + "% " + lang.off }} </span>
							<span v-if="product.current_stock == 0" class="base stock_badge">Out Of Stock</span>
							<span class="base reword-badge" v-if="product.reward > 0">{{ lang.reward_point }}: {{ product.reward }}</span>
							<img :src="product.media[0].url" :alt="product.name" class="img-fluid" loading="lazy" style="object-fit: cover;width: 197px;height: 200px"/>
						</div>
					</a>
					<div class="product-info">
						...
						<h1 class="product-name text-ellipse-one" :title="product.name">
							<a :href="getUrl('product/' + product.id)" @click.prevent="routerNavigator('product.details', product.id)">
								{{ product.name }}
							</a>
						</h1>
						<div class="sg-rating">
							<star-rating v-model:rating="product.rating" :read-only="true" :star-size="12" :round-start-rating="false"></star-rating>
							<span class="reviews" v-if="product.reviews_count > 0">({{ product.reviews_count }} Reviews)</span>
						</div>
						<div class="icons">
