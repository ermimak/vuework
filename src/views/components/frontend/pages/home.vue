<template>
  ...
  <div v-if="siteSettings">
    <slider v-if="offers" :sliders="offers" :addons="siteSettings.addons"></slider>
    <services v-if="siteSettings.settings.show_service_info_section == 1 " :services="siteSettings.services"></services>
    ...
    <viewed_product v-if="siteSettings.settings.recent_viewed == 1 " :settings="siteSettings.settings" :addons="siteSettings.addons"></viewed_product>
    <subscribe v-if="siteSettings.settings.subscription_section && siteSettings.settings.subscription_section == 1"></subscribe>
    <deal v-if="siteSettings.settings.article_section && siteSettings.settings.article_section == 1"></deal>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getOffers(){
      axios.get(`${this.url2}offers`).then((response) => {
        this.is_shimmer = true
        if (response.data.error) {
          this.$Progress.fail();
          toastr.error(response.data.error, this.lang.Error + ' !!');
