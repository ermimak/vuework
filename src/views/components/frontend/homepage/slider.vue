							...
							<VueSlickCarousel class="hero-section" :class="{ 'ishopet-hero-section' : addons.includes('ishopet') }" v-bind="slick_settings" :rtl="settings.text_direction == 'rtl'">
								<div class="hero-slide-item" v-for="(slider, i) in sliders" :key="i">
									<div class="slider__img slider_div">
										<a :href="slider.media[1].url" v-if="urlCheck(slider.media[1].url)">
											<img :src="slider.media[1].url" :alt="slider.media[1].url" />
										</a>
										<a href="javascript:void(0)" v-else-if="!slider.media[1].url">
											<img :src="slider.media[1].url" :alt="slider.link" />
										</a>
										<router-link v-else :to="slider.media[1].url"><img :src="slider.media[1].url" :alt="slider.media[1].url" /></router-link>
									</div>
								</div>
							</VueSlickCarousel>
						</div>
